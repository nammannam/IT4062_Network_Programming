...
AccountInfo_s searchUsernameList(Llist_s *list, char *username){
    ...
    Llist_s *pt;
    printf("[DEBUG searchUsernameList] Looking for username: '%s'\n", username);
    int count = 0;
    for(pt = list ; pt != NULL ; pt = pt->next){
        if (pt->nodeInfo.username[0] != '\0') {
            printf("[DEBUG searchUsernameList] Entry %d: '%s'\n", count++, pt->nodeInfo.username);
        }
        if(strcasecmp(pt->nodeInfo.username, username) == 0){
            printf("[DEBUG searchUsernameList] FOUND: '%s'\n", username);
            return pt->nodeInfo;
        }
    }
    printf("[DEBUG searchUsernameList] NOT FOUND: '%s'\n", username);
