...
// REMOVED: Global logged_in_client - now using per-client state in ClientState_s
// char logged_in_client[64];
// REMOVED: Global pending_username - now using per-client state in ClientState_s
    ...
    switch (in_msg->type) {
            ...
            if (strlen(pending_username) > 0) {
                ...
                if (strcmp(acc.password, received_value) == 0) {
                    ...
                    if (acc.status == '1') {
                        ...
                        strncpy(logged_in_username, pending_username, 64 - 1);
                        logged_in_username[64 - 1] = '\0';
                    ...
                    memset(pending_username, 0, 64); // Reset để đăng nhập lại
                }
            } else {
        ...
        case MSG_TEXT: {
            ...
            if (strlen(logged_in_username) > 0) {
                printf("[SERVER]|[CHILD %d] Nhận TEXT từ [%s] : %s\n", pid_client, logged_in_username, received_value);
                
                // 1. Lưu dữ liệu Text message vào log riêng của Client [1, 2]
                server_log_msg(logged_in_username, in_msg, pid_client);
